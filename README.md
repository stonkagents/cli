...
`stonkagents` is the command-line agent runtime behind [StonkAgents](https://stonkagents.com). It is a fork of [OpenClaw](https://github.com/openclaw/openclaw), the MIT-licensed multi-channel AI gateway, shipped with the StonkAgents AI provider plugin and the pieces the StonkAgents installer relies on. Everything from the OpenClaw heading down is OpenClaw's own README, kept as it is upstream.

- Documentation: [docs.stonkagents.com](https://docs.stonkagents.com)
- Source: [github.com/stonkagents/cli](https://github.com/stonkagents/cli)
- Upstream: [github.com/openclaw/openclaw](https://github.com/openclaw/openclaw) (see [NOTICE](NOTICE) for attribution)
- Contributing: [CONTRIBUTING.md](CONTRIBUTING.md), security reports: [SECURITY.md](SECURITY.md)

Generic fixes belong upstream in OpenClaw first; this repository carries the StonkAgents-specific parts and tracks upstream releases.

## What differs from OpenClaw

- `extensions/stonkagents/`: the bundled **StonkAgents AI** model provider. It routes completions through the StonkAgents tracker, which meters credits and proxies to an upstream LLM, so users never buy LLM credits directly.
- The `stonkagents` binary (alongside `openclaw`) and a package-root resolver that accepts the `stonkagents` package name.
- Small onboarding and CLI adjustments so the installer can run non-interactively.

## Quick start

The installer (`.exe` / `.dmg`) sets `STONKAGENTS_API_KEY` and `STONKAGENTS_TRACKER_URL`, then runs:

```bash
stonkagents onboard --auth-choice=stonkagents-ai --accept-risk --non-interactive
```

For interactive use:

```bash
export STONKAGENTS_API_KEY="your-stonkagents-api-key"
export STONKAGENTS_TRACKER_URL="https://tracker.dev.stonkagents.com"
stonkagents onboard
# Pick "StonkAgents AI" from the auth choice list.
```

Or via a CLI flag:

```bash
stonkagents onboard --stonkagents-api-key "your-key"
```

The tracker URL has no CLI flag; set `STONKAGENTS_TRACKER_URL` in the environment instead.

### Gateway dashboard (Control UI)

After running `stonkagents gateway`, open **http://127.0.0.1:18789** in your browser. If the dashboard shows **"gateway token missing"** or "unauthorized":

1. Get the token: `stonkagents config get gateway.auth.token`
   (If empty, generate one: `stonkagents doctor --generate-gateway-token`.)
...
| ------------------------- | ------------------------------------------------ | ------------------------------------- |
| `STONKAGENTS_API_KEY`     | StonkAgents API key (issued by the tracker)      | none                                  |
| `STONKAGENTS_TRACKER_URL` | Tracker base URL for StonkAgents LLM completions | `https://tracker.dev.stonkagents.com` |

Completions go to `<tracker>/api/v1/agents/chat/completions`.

### Installer contract

For an installer that runs StonkAgents onboarding:

1. Set `STONKAGENTS_API_KEY` (obtained from the StonkAgents agent's `GET /api/v1/installer/peer-key` endpoint, or from the tracker's `POST /api/v1/portal/guest-key`).
2. Set `STONKAGENTS_TRACKER_URL` to the tracker URL returned alongside the key.
3. Run: `stonkagents onboard --auth-choice=stonkagents-ai --accept-risk --non-interactive`
4. After onboarding, start the gateway: `stonkagents gateway`.
...
Node 22.14 or newer and pnpm 10 are required (see `packageManager` in `package.json`).
