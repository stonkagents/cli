...
describe("resolveOpenClawPackageRoot", () => {
  ...
  it.each([
    ...
    ...["stonkagents"].map((npmName) => ({
      name: `resolves package root when published as ${npmName}`,
      setup: () => {
        const project = fx(`${npmName}-scenario`);
        const argv1 = path.join(project, "node_modules", ".bin", npmName);
        const pkgRoot = path.join(project, "node_modules", npmName);
        setPackageRoot(pkgRoot, npmName);
        return { opts: { argv1 }, expected: pkgRoot };
      },
    })),
    {
