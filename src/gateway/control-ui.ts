...
export type ControlUiRequestOptions = {
  ...
  /** When set (only for loopback requests), included in the bootstrap JSON so the UI can auto-connect without the user pasting the token. */
  gatewayToken?: string;
};
...
export function handleControlUiHttpRequest(
  ...
  if (pathname === bootstrapConfigPath) {
    ...
    if (typeof opts?.gatewayToken === "string" && opts.gatewayToken.trim()) {
      bootstrapPayload.gatewayToken = opts.gatewayToken.trim();
    }
