    ...
    try {
      ...
      if (controlUiEnabled) {
        ...
        requestStages.push({
          ...
          run: () => {
            const controlUiOpts: import("./control-ui.js").ControlUiRequestOptions = {
              basePath: controlUiBasePath,
              config: configSnapshot,
              root: controlUiRoot,
            };
            // StonkAgents: inject gateway token for loopback requests so the
            // dashboard auto-connects without the user pasting the token.
            if (
              isLocalDirectRequest(req, trustedProxies, allowRealIpFallback) &&
              resolvedAuth.token
            ) {
              controlUiOpts.gatewayToken = resolvedAuth.token;
            }
            return handleControlUiHttpRequest(req, res, controlUiOpts);
          },
        });
      }
